...
test.describe('Chat Flow', () => {
  ...
  test('should send message and display response', async ({ page }) => {
    ...
    // 验证用户消息容器存在且在视口内
    const userMessageContainer = page.locator('.flex.gap-3.flex-row-reverse').first()
    await expect(userMessageContainer).toBeAttached()
    await userMessageContainer.scrollIntoViewIfNeeded()
    await expect(userMessageContainer).toBeInViewport()

    // 验证消息容器包含正确的文本内容
    await expect(userMessageContainer.getByText('画一个简单的计数器')).toBeAttached()
  ...
  test('should handle Enter key to send message', async ({ page }) => {
    ...
    // 验证消息容器包含正确的文本内容
    await expect(userMessageContainer.getByText('测试 Enter 键发送')).toBeAttached()
  ...
  test('should display message bubbles with correct styling', async ({ page }) => {
    ...
    // 验证用户消息包含正确的内容（文本存在于 DOM 中）
    await expect(userMessageContainer.getByText('测试消息样式')).toBeAttached()

    // 验证用户消息有用户图标
    await expect(userMessageContainer.locator('svg')).toBeAttached()
  })
